...
// [[Rcpp::export]]
void heavy_parameter_transformR(NumericVector parameters, int K, IntegerVector p, IntegerVector q, NumericVector O,
...
// [[Rcpp::export]]
void heavy_likelihoodR(NumericVector parameters, NumericVector data, IntegerVector T1, IntegerVector K1, NumericVector means, IntegerVector p,
                       IntegerVector q, IntegerVector pMax1, IntegerVector qMax1,
                       NumericVector backcast, NumericVector LB, NumericVector UB, IntegerVector compconst1,
                       NumericVector h, NumericVector lls, NumericVector llRM, NumericVector ll) {
    ...
    for (j = 0; j < qMax; j++) {// j=1:pMax
      if (t-j > 0) {
        ...
        for (l = 0; l < K; l++) {
          ...
          } //{sum = sum + B[K*K*j + K*l + k]*GETM(model->h,t-j-1,k);}
          //htemp = GETM(h,l,t);
          //temp[l] = sum;
          //printf("%lf\n", temp[l]);
