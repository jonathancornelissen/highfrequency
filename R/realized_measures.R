...
#' 
#' overlapR <- function(min1, max1, min2, max2){
#'   return(max(0, min(max1, max2) - max(min1, min2)) > 0)
#' }
#' 
#' 
#' #' @export
#' leadLagR <- function(price1, price2, lagRange, normalize = FALSE){
#'   
...
#'       ret <- x[i+1] - x[i]
#'       
#'       midPointBackup <- findFirst(clampedTimestamps, timestampsX[i]) +1 #plus 1 because R is 1-indexed
#'       
#'       midPoint <- midPointBackup 
#' 
#'       while(TRUE){
#'         if(midPoint < 1 | midPoint >= length(timestampsY)){
#'           break
#'         }
#'         j0 <- midPoint
#'         j1 <- midPoint + 1
...
#'     }
#'     
#'     idx <- idx + 1 ## increment the index of our results
#'   }
#'   
#'   
#'   
...
#' @export
leadLag <- function(price1, price2, lagRange, normalize = TRUE){
  ...
  x <- as.numeric(price1)
  y <- as.numeric(price2)
  contrasts <- leadLagCpp(x, timestampsX, y, timestampsY, lagRange, normalize)
