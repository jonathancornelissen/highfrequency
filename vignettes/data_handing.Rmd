...
```{r eval = TRUE}
summary(sampleTDataRaw_microseconds[, c("DT", "SIZE", "PRICE")])
```
...
```{r eval = TRUE}
tdata_cleaned <- tradesCleanup(tdataraw = sampleTDataRaw_microseconds, exchange = "N")
```
...
```{r eval = TRUE}
qdata_cleaned <- quotesCleanup(qdataraw = sampleQDataRawMicroseconds, exchange = "N")
```
