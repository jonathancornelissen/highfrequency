...
#'
#' @examples
#' ##### Example 1: Rolling mean and median estimators for 2 day #####
#' data("sample_tdata_microseconds")
#' dat = sample_tdata_microseconds[,SYMBOL:=NULL]
#' meandrift = highfrequency:::spotdrift(data = dat, k = 1, tz = "EST")
#' mediandrift = highfrequency:::spotdrift(data = dat, method = "driftMedian",k = 1, tz = "EST")
#' plot(meandrift)
#' plot(mediandrift)
#'
#'
...
spotdrift <- function(data, method = "driftMean", ..., on = "minutes", k = 5,
  ...
  options <- list(...)
  out <- switch(method,
                driftKernel = driftKernel(data = data, delta, intraday, options), ###works only for one day at a time! Does the rest of data preparation in the function
...
#########Helper function taken from the old R code in spotvol.R #########
#' @keywords internal
#' @importFrom xts last
aggregatePrice = function (ts, on = "minutes", k = 1, marketopen = "09:30:00", marketclose = "16:00:00" , tz = "GMT") {
